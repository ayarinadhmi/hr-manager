...
class HrManagerKafkaResourceIT {
    ...
    void setup() {
        ...
        HrManagerKafkaResource kafkaResource = new HrManagerKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc
            .perform(post("/api/hr-manager-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/hr-manager-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
